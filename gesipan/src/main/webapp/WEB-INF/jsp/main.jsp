...
<div id="wrapper" class="divided">
    ...
    <c:forEach var="board" items="${list}" varStatus="loop">
        <c:if test="${board.boardType eq 'story'}">
            ...
            <c:if test="${loop.index % 2 ne 0}">
                ...
                <div class="content">
                    <h2>${board.title}</h2>
                    <p>This is also a <strong>${board.content}</strong> element, and it's here because this demo would look a bit empty with just one spotlight. Like all spotlights, you can customize its <span class="demo-controls">appearance with a number of modifiers</span>, as well as assign it an optional <code>onload</code> or <code>onscroll</code> transition modifier (<a href="#reference-spotlight">details</a>).</p>
                    <ul class="actions stacked">
    ...
    <section class="wrapper style1 align-center">
        ...
        <c:if test="${board.boardType eq 'gallery'}">
            <article>
    ...
    <section class="wrapper style1 align-center">
        <div class="inner">
            ...
            <c:forEach var="board" items="${list}">
                <c:if test="${board.boardType eq 'items'}">
                <section>
        ...
        <div class="inner">
            ...
            <div class="index align-left">
                ...
                <section>
                    ...
                    <div class="content">
                        <c:forEach var="board" items="${list}">
                        <c:if test="${board.boardType eq 'headings'}">

                        <header>
                            <h2>${board.title}</h2>
                            <p>${board.subContent}</p>
                        </header>
                        <p>${board.content}</p>
                ...
                <section>
                    ...
                    <div class="content">
                        <dl>
                            <c:forEach var="board" items="${list}">
                            <c:if test="${board.boardType eq 'definitionList'}">
                            <dt>${board.title}</dt>
                            <dd>
                                <p>${board.content}</p>
                            </dd>
